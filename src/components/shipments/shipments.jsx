...
const Shipments = () => {
  ...
  const [dateSort, setDateSort] = useState(0); // 0off 1asc 2desc
  useEffect(() => {
    axios
      ...
      .catch((err) => {
        console.log(err);
        axios
          .get("http://localhost:3000/Shipments.txt")
          .then((res) => {
            setShipments(res.data);
          })
  ...
  return (
    <>
      <header className="headerWrapper">
        <div className="titleDiv">Shipments</div>
      </header>
      <Shipment
      ...
      <table className="app--table">
        <thead className="app--thead">
          <tr className="app--tr">
            ...
            <th className="app--th-date" onClick={dateSortHandler}>
              date
            </th>
        ...
        <tbody className="app-tbody">
          {shipments.map((shipment) => (
            <tr className="app--tr" key={shipment.orderNo}>
              ...
              <td className="app--td-button">
                <button
                  className="app--button-delete"
                  onClick={() => deleteHandler(shipment.orderNo)}
                >
